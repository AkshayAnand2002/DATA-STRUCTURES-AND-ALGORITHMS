...
If the operator has precedence lesser than the top of the stack, pop the operator and output it to the prefix notation output and 
then check the above condition again with the new top of the stack.
...
//method that pushes elements onto the character stack.
void push(int pos){
	if(top==MAX-1){
		printf("\nSTACK OVERFLOW\n");
	}
...
//method to remove character from stack and return them.
char pop(){
	char ch;
if(top < 0)
{
